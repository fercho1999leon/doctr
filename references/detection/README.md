...
### 2b. Alternative detector: LW-DETR (object detection)

`convert_documentai.py` also writes `labels_layout.json` in every split: the same boxes in the format of `references/layout/train.py`. LW-DETR (`lw_detr_s`, `lw_detr_m`) is a DETR-style object detector pretrained on DocLayNet that predicts one box per region with its class, which suits block-like fields (multi-line addresses/accounts) better than a segmentation model that has to regroup lines:

```shell
python references/layout/train.py lw_detr_s --pretrained \
  --train_path data/fields/train --val_path data/fields/val --labels-name labels_layout.json \
  --device 0 --amp -b 8 --epochs 100 --no-hflip --early-stop --early-stop-epochs 15 \
...
The report also sweeps a **minimum detection score per class** on the split and prints the values that maximise F1 (a field detected on a page where it is absent counts as a false positive). Apply them with `--min-score numero_control=0.62 ...` in both `evaluate_fields.py` and `kie_inference.py`; tune on `val`, then run `test` once. `--merge-fragments` joins vertically adjacent regions of the same class before keeping the best one, which helps when a multi-line field is detected line by line. `--straighten` rotates photos and scans upright before detection (doctr's page orientation classifier for 0/90/180/270, refined with the skew of the text lines): the detector is trained on upright screenshots and returns nothing on a page photographed sideways. It is a no-op on upright pages, so it is safe to enable in production. `--fallback` searches the OCR lines of the whole page by pattern (masked account, `No.` number, `$` amount, date, `control` number) when the detector returns nothing for a field; such values carry `"source": "fallback"` in the output. The key match in the report is geometry-free (the value returned for the field is what is scored), the detection P/R/F1 columns still use IoU. The date key is the day only; `extract_time` returns the time when present. The report ends with a diagnosis and a list of recommendations derived from the metrics (train longer, tune thresholds, add data for a class, fine-tune the recogniser, ...). A Markdown and a JSON report are written next to the data.
